...
> **追記**: レガシー mipmap の出荷形式はその後 PNG に変更した。Firebase App
> Distribution が WebP を解釈できずアイコンを表示できなかったため。以下の本文に
> 残る `webp` の記述は当時の計画であり、現行の手順は
> `docs/superpowers/assets/icon/README.md` を参照すること。
...
テーマアイコンは drawable のアルファのみを使って単色で塗り潰す。したがってカラー版の前景を流用すると顔も消える。目・口は **逆回りのサブパスによる nonZero の穴** として表現する（`evenOdd` にすると羽根と胴体の重なりまで穴になるため使わない）。縞は単色シルエットにするとひげのように見えてしまうため、モノクロレイヤーでは持たない。触角と羽根は穴を持たないので、胴体とは別のパスに分ける。羽根は胴体楕円を 2 単位膨らませた領域を `clip` で欠き取り、胴体と地続きの塊に見えないよう隙間を空ける。
...
1. 単色に潰しても 目 2 つ・口 が穴として残っている（縞は持たない）
2. 48px（右）でも目と口が判別できる
3. 羽根と胴体の間に隙間が見え、羽根が胴体と地続きの塊（マッシュルーム状）に見えない
...
| monochrome 専用 drawable、目・口を穴に、羽根と胴体の間に隙間 | Task 3 |
